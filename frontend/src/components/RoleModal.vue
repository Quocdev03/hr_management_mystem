        ...
        if (newVal) {
            ...
            if (props.mode === "edit" && props.roleData) {
                formData.value = {
                    ...
                    permissions: props.roleData.permissions ? [...props.roleData.permissions] : [],
                };
            } else {
                formData.value = {
                    name: "",
...
const isSystemRole = computed(() => {
    ...
    return props.mode === 'edit' && (name === 'admin' || name === 'hr' || name === 'employee');
});

const isAdminRole = computed(() => {
    return props.mode === 'edit' && formData.value.name.toLowerCase() === 'admin';
});
...
<template>
    <ModalDialog :visible="modelValue" @close="$emit('update:modelValue', false)"
        :title="mode === 'create' ? 'Thêm vai trò mới' : 'Chỉnh sửa vai trò'" width="max-w-2xl">
        <form @submit.prevent="handleSubmit" class="form-wrapper">
            ...
            <div class="form-group form-group--full">
                ...
                <input v-model="formData.name" type="text" :disabled="isSystemRole"
                    class="form-control"
                    placeholder="Ví dụ: Quản lý chi nhánh" />
                <p v-if="isSystemRole" class="helper-text helper-text--warning">Không thể đổi tên vai trò hệ thống.</p>
            </div>
            ...
            <div class="form-group form-group--full">
                ...
                <textarea v-model="formData.description" rows="2"
                    class="form-control"
                    placeholder="Mô tả chức năng của vai trò này"></textarea>
            </div>
            ...
            <div class="permission-section form-group--full">
                ...
                <div v-if="isAdminRole" class="admin-notice">
                    Vai trò <strong>Admin</strong> mặc định có toàn quyền. Bạn không cần phải tick thủ công.
                </div>

                <div class="permission-list custom-scrollbar">
                    <div v-for="(perms, moduleName) in groupedPermissions" :key="moduleName" class="permission-group">
                        ...
                        <div class="permission-grid">
                            <label v-for="perm in perms" :key="perm.code" class="permission-item">
                                <input type="checkbox" :value="perm.code" v-model="formData.permissions"
