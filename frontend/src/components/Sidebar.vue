<template>
	<aside class="sidebar" :class="{ 'is-open': isOpen }">
		<nav class="sidebar-nav">
			...
			<router-link v-if="canManageUsers" to="/users" class="nav-item" active-class="active">
...
<style scoped>
	...
	.nav-icon {
		width: 20px;
		height: 20px;
		filter: invert(45%) sepia(12%) saturate(545%) hue-rotate(182deg) brightness(92%)
			contrast(89%); /* var(--text-muted) */
		transition: all 0.2s ease;
		z-index: 1;
	}
	...
	.nav-item:hover .nav-icon {
		filter: invert(13%) sepia(15%) saturate(1487%) hue-rotate(182deg) brightness(98%)
			contrast(92%); /* var(--text-main) */
	}
