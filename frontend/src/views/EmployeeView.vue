<script setup>
   ...
   async function syncDepartmentManager(employee, isManagerSelected) {
      if (!isManagerSelected || !employee || !employee.id || !employee.department_id) {
         return;
      }
      ...
      const res = await departmentStore.updateDepartment(employee.department_id, {
         manager_id: employee.id,
      });

      if (res.success === false) {
         throw new Error(res.message || "Không thể đồng bộ trưởng phòng");
      }
   }
   ...
   async function onFormSubmit(formData) {
      ...
      try {
         await syncDepartmentManager(res.data, formData.is_manager);
      } catch (err) {
         console.error(err);
         toast.error(err.message || "Không thể đồng bộ trưởng phòng");
      }
               ...
               <tbody>
                  ...
                  <tr v-if="employees.length === 0">
                     <td colspan="6" class="empty-state">
                        ...
                        <p class="empty-state__text">Không có dữ liệu nhân viên nào phù hợp.</p>
                     </td>
                  </tr>
               </tbody>
            </table>
