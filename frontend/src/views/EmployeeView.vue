<script setup>
	...
	async function loadFormRelations() {
		try {
			await Promise.all([
	...
	async function handleFormSubmit() {
		...
		if (isEditMode.value === true) {
			// Edit mode: dirty check payload
			const original = buildFormData(editingEmployee.value);
			const payload = Object.fromEntries(
				Object.keys(original)
					.filter((k) => formData.value[k] !== original[k])
			...
			res = await employeeStore.updateEmployee(editingEmployee.value.id, payload);
		} else {
			// Create mode
			const data = { ...formData.value };
			data.user_id =
				data.user_id !== "" && data.user_id != null ? Number(data.user_id) : null;
			data.department_id =
		...
		>
			<form @submit.prevent="handleFormSubmit" class="employee-form">
				<div class="form-grid">
					...
					<div class="form-group" :class="{ 'form-group--disabled': isEditMode }">
				...
				<div class="form-group">
					...
					<select v-model="formData.user_id" class="form-control">
						...
						<option v-if="editingEmployee?.user" :value="editingEmployee.user.id">
							{{ editingEmployee.user.email }} (hiện tại)
						</option>

						<!-- danh sách user chưa có employee -->
						<option v-for="u in usersWithoutEmp" :key="u.id" :value="u.id">
							{{ u.email }}
						</option>
					</select>
				</div>
